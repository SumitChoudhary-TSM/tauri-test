...
- Use **Google Icons** instead:
  ...
  ```tsx
  export const KeyboardArrowUpIcon: React.FC<IconProps> = ({ color, size = 24, className }) => (
    <svg
...
When working with colors in JavaScript or TypeScript files:
...
- Use them in your components:
  ```tsx
  <div style={{ color: defaultColors.PRIMARY }}>Red text</div>
  ```
- Never hardcode color hex values in your components.

### CSS/SCSS

When working with colors in CSS or SCSS files:

- Use the CSS variables defined in the variables file:
  ```scss
  .my-element {
    color: var(--primary);
  ...
  ```
- Alternatively, you can use the SCSS variables directly in .scss files:
  ```scss
  .my-element {
    color: $primary-color;
